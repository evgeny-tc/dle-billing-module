...
try
{
    ...
    if( $group_settings['type'] )
    {
        ...
        foreach( $arPrices as $price_str )
        {
            ...
            $_tpl_select_buffer = str_replace('{price}', \Billing\Api\Balance::Init()->Convert( floatval($price_ex[2]) ), $_tpl_select_buffer);
            $_tpl_select_buffer = str_replace('{currency}', \Billing\Api\Balance::Init()->Declension( floatval($price_ex[2]) ), $_tpl_select_buffer);
    ...
    $tpl->set( '{pay.sum}', \Billing\Api\Balance::Init()->Convert( floatval($_Price) ) );
    $tpl->set( '{pay.sum.currency}', \Billing\Api\Balance::Init()->Declension( floatval($_Price) ) );
