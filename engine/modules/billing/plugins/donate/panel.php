...
else
{
    ...
    # Логин
    #
    $login = strtr($login, array_flip(get_html_translation_table(HTML_ENTITIES, ENT_QUOTES, $config['charset'])));
    ...
    if( file_exists( ROOT_DIR . '/templates/' . $config['skin'] . '/billing/plugins/donate/' . $tpanel . '.tpl' ) )
    {
        ...
        # Теги
        #
        $_Content = str_replace('{panel-id}', str_replace('%', '', urlencode( base64_encode($login . $code) )), $_Content);
