...
$_Config = include MODULE_DATA . '/plugin.forms.php';
$_Lang = include MODULE_PATH . '/plugins/forms/lang.php';
$_ConfigBilling = include MODULE_DATA . '/config.php';

$_hash = sha1( SECURE_AUTH_KEY . ( $member_id['name'] ? $member_id['user_id'] . sha1($member_id['password']) : $_SERVER['REMOTE_ADDR'] ) . $member_id['hash'] );

if( file_exists( MODULE_DATA . "/plugin.forms.php" ) )
{
    $_Config = include MODULE_DATA . "/plugin.forms.php";
}
...
# Check hash
#
if( empty($Data['hidden_params[dle_login_hash]']) or $Data['hidden_params[dle_login_hash]'] != $_hash )
{
    billing_error( $_Lang['errors']['hash'] );
}
